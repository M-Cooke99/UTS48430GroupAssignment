...
void createCode (min_heap_node_t* node, int code[], int current_node);

/* apply the huffman coding on data */
void HuffmanCompression(char data[], int frequency[], int size);
...
int main (void) {
    ...
    HuffmanCompression(data, frequency, size); 
...
void createCode (min_heap_node_t* node, int code[], int current_node) {
    if (node->left) {
        ...
        code[current_node] = 0;
        createCode(node->left, code, (current_node+1));
    }

    if (node->right) {
        /* assign 1 to the right node */
        code[current_node] = 1;
        createCode(node->right, code, (current_node+1));
    }    
...
void HuffmanCompression(char data[], int frequency[], int size) {
    min_heap_node_t* root = buildHuffmanTree(data, frequency, size);

    int code[100];
    int current_node = 0;

    createCode(root, code, current_node);
}
